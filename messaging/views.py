...
class MessageDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_object(self):
        ...
        if not obj:
            ...
            raise NotFound(detail="""Message does not exist or
        ...
        elif (self.request.method == 'GET' and obj.recipient
              == self.request.user and not obj.read):
            obj.read = True
            obj.save()
    ...
    def perform_update(self, serializer):
        ...
        if (timezone.now() - serializer.instance.created_at >
           timedelta(hours=24)):
            raise PermissionDenied("""Messages can only be edited within
            24 hours of sending.""")
        serializer.save(is_edited=True)

    def perform_destroy(self, instance):
        # Check if the message can be deleted based on
        # your logic (e.g., within 24 hours)
        if timezone.now() - instance.created_at > timedelta(hours=24):
            raise PermissionDenied("""Messages can only be deleted within 24
            hours of sending.""")
        instance.delete()
...
class ConversationListView(generics.ListAPIView):
    ...
    permission_classes = [permissions.IsAuthenticated]  # Ensure only
    ...
    def get_queryset(self):
        ...
        if user.is_authenticated:
            # Filter conversations to only those where the
            # current user is a participant
            return Conversation.objects.filter(
                                                participants__in=[user]
                                                ).distinct()
        else:
            # Optionally handle unauthenticated users, such as
